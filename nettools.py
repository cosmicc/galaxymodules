"""
 ...
 various custom network functions created for my own use

 is_valid_ipv4_address()
 to_ip()
 ping() - dict returned of ping results
 externalip() - DEPRECIATED use internet_info()
 isup.host_osping() - bool returned wether host is alive with an os initiated ping
 isup.host_udp() - bool returned wether host is alive with an udp ping
 isup.host_icmp() - bool returned wether host is alive with an icmp ping
 isup.host_port() - bool returned wether host port is alive
 isup.internet() - bool returned wether the internet is accessable (dns query)
 isup.httpout() - bool returned wether http is allowed to the internet
 isup.icmpout() - bool returned wether icmp is allowed to the internet
 internet_info() - dict returned with server and client information
 internet_speedtest() - dict returned with same info plus down/up speeds in kBps
 getip_from_host() - str returned of ip address from hostname
 getips_from_domain() - list containing one or more IP address strings that respond as the given domain name
 gethost_from_ip() -  str returned the 'True Host' name for agiven IP address
 getalias_from_domain() - array returned containing a list of aliases for the given domain
"""
...
def to_ip(addr):
    if is_valid_ip4_address(addr):
        return addr
    try:
        resaddr = socket.gethostbyname(addr)
    except:
...
class Ping(object):
    ...
    def print_exit(self):
        ...
        log.info('Ping results: {}'.format(msg))
        if self.quiet_output:
            self.response.output.append(msg)
...
def externalip():
    try:
        ...
        log.debug('External IP resolver successfully found IP address {}'.format(external_ip))
        return external_ip
    except:
        log.warning('External IP resolver failed.')
        return false
'''
...
def internet_speedtest(mode=False):
    try:
        ...
        return_dict['server_sponsor'] = results_dict['server']['sponsor']
        return_dict['server_city'] = results_dict['server']['name']
        return_dict['server_country'] = results_dict['server']['country']
        return_dict['client_ip'] = results_dict['client']['ip']
        return_dict['client_country'] = results_dict['client']['country']
        return_dict['client_isp'] = results_dict['client']['isp']
        return_dict['client_isprating'] = results_dict['client']['isprating']
        return_dict['client_lat'] = results_dict['client']['lat']
        return_dict['client_lon'] = results_dict['client']['lon']
    except:
        return False
    else:
        return return_dict
...
class isup:
    def host_osping(host):
        if os.name == 'posix':
            pingresults = os.system('ping -c 1 -w 2 ' + host + ' >/dev/null 2>&1')
        elif os.name == 'nt':
            pingresults = os.system('ping -n 1 -w 2000 ' + host + ' > nul 2>&1')
        else:
            log.error('host_osping does not support this system type')
            exit()
        if pingresults == 0:
            log.debug('host_osping successfull to host {}'.format(host))
            return True
        else:
            log.debug('host_osping filed to host {}'.format(host))
            return False
    ...
    def host_udp(host):
        ...
        if g.ret_code == 0:
            ...
            log.debug('host_udp filed to host {}'.format(host))
            return False
    ...
    def host_icmp(host):
        ...
        if g.ret_code == 0:
            ...
            log.debug('host_icmp filed to host {}'.format(host))
            return False
    ...
    def internet():
        if isup.host_icmp('1.1.1.1'):
            log.debug('Internet check returned TRUE')
            return True
        else:
            log.debug('Internet check returned FALSE')
            return False

    def httpout():
        if isup.host_port('www.google.com', 80):
            log.debug('HTTP out check returned TRUE')
            return True
        else:
            log.debug('HTTP out check returned FALSE')
            return False
    ...
    def icmpout():
        if isup.host_icmp('8.8.8.8') is True:
            ...
            log.debug('ICMP out check returned FALSE')
            return False


def getip_from_host(d):
    try:
